...
class SketchReport:
    ...
    def invalid_interior_vertices(self) -> tuple[VertexConfiguration, ...]:
        return tuple(
            ...
            if _is_interior_point(
                vertex.position,
                self.patch_bounds,
                cell_width=self.cell_width,
                cell_height=self.cell_height,
                lattice_skew_x=self.lattice_skew_x,
            )
...
def sketch(input_data: SketchInput, *, patch_size: int = 3) -> SketchReport:
    ...
    unmatched_edges = tuple(
        ...
        if (
            (
                len(set(cell_ids)) == 1
                and _is_interior_edge(
                    key,
                    logical_bounds,
                    cell_width=input_data.cell_width,
                    cell_height=input_data.cell_height,
                    lattice_skew_x=input_data.lattice_skew_x,
                )
            )
    ...
    t_junctions = _check_t_junctions(
        cells,
        logical_bounds,
        cell_width=input_data.cell_width,
        cell_height=input_data.cell_height,
...
def _is_interior_point(
    ...
    *,
    ...
    """Return whether a point is strictly inside the logical generated patch."""

    min_x, min_y, max_x, max_y = patch_bounds
    margin = 1e-3
    if lattice_skew_x is None:
        return (
    ...
    # A cumulative-skew patch is a parallelogram generated by basis vectors
    # (cell_width, 0) and (lattice_skew_x, cell_height). Classify interior in
    # those lattice coordinates, otherwise valid skew-boundary straddlers are
    # falsely reported as holes against the axis-aligned bounding box.
    relative_y = point[1] - min_y
    lattice_y = relative_y / cell_height
    relative_x = point[0] - min_x
    lattice_x = (relative_x - lattice_y * lattice_skew_x) / cell_width
    patch_width = (max_x - min_x) / cell_width
    patch_height = (max_y - min_y) / cell_height
    lattice_margin = margin / max(min(abs(cell_width), abs(cell_height)), 1.0)
    return (
        lattice_margin < lattice_x < patch_width - lattice_margin
        and lattice_margin < lattice_y < patch_height - lattice_margin
    )
...
SPECS = {{
    "{input_data.geometry}": ReferenceFamilySpec(
        ...
        root_seed_policy="descriptor-driven open-boundary {patch_size}x{patch_size} sample",
