    ...
    if parsed_values is None:
        ...
        for value in parsed_values:
            if value <= 0 or value > 1:
                _parser_error(parser, "--values must contain cleanup scales in the interval (0, 1].")
...
def _cleanup_warnings(
    *,
    ...
    if overlap_pair_count == 0 and (
        ...
        warnings.append("Cleanup removed overlap but introduced comparatively large bounds drift versus the shipped baseline.")
    ...
    if browser_review_summary is None:
        ...
        if not isinstance(visual_metrics, dict) or visual_metrics.get("gutterScore") is None:
            warnings.append("Visible gutter risk could not be derived from the browser review metrics.")
    return warnings
            ...
            manifest["candidates"].append(
                build_candidate_manifest_record(
                    ...
                    key_metrics={
                        ...
                        "overlapPairCount": structural_summary["cleanupDiagnostics"]["overlapPairCount"],
                        "maxOverlapArea": structural_summary["cleanupDiagnostics"]["maxOverlapArea"],
