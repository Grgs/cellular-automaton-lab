...
def ensure_render_review_outputs(
    ...
    *,
    ...
    if (
        review_args.out is not None
        and review_args.summary_out is not None
        and (
            (
                review_args.reference is None
                and not bool(getattr(review_args, "literature_review", False))
            )
    ...
    if (
        review_args.reference is not None or bool(getattr(review_args, "literature_review", False))
    ) and review_args.montage_out is None:
...
def run_managed_render_review(
    *,
    ...
    try:
        ...
        run_manifest["settleDiagnostics"] = condense_settle_diagnostics(
            summary_payload.get("settleDiagnostics")
        )
        run_manifest["transformSummary"] = condense_transform_report(
            summary_payload.get("transformReport")
        )
        run_manifest["overlapHotspots"] = condense_overlap_hotspots(
            summary_payload.get("overlapHotspots")
        )
        run_manifest["visualMetrics"] = condense_visual_metrics(
            summary_payload.get("visualMetrics")
        )
...
def run_unittest_with_managed_host(
    *,
    ...
    _write_host_logs(
        artifact_dir, stdout_text=stdout_text or "", stderr_text=stderr_text or "", stem="unittest"
    )
