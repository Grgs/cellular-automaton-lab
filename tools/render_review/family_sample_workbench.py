...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--values", help="Comma-separated candidate values for the selected strategy."
    )
    parser.add_argument(
        "--browser-review",
        action="store_true",
...
def default_shield_window_values(*, patch_depth: int) -> tuple[float, ...]:
    baseline_threshold = json.loads(
        (ROOT_DIR / "backend" / "simulation" / "data" / "shield_reference_patch.json").read_text(
            encoding="utf-8"
        )
    )["representative_window_thresholds"][str(max(0, int(patch_depth)))]
    return tuple(
        round(float(baseline_threshold) * multiplier, 6)
        for multiplier in DEFAULT_SHIELD_WINDOW_MULTIPLIERS
    )
        ...
        json.dumps(
            {
                ...
                "parameterValue": round(parameter_value, 6)
...
def build_structural_summary(
    *,
    ...
    graph_component_count = (
        0 if validation.checked_cell_count == 0 else 1 + len(validation.disconnected_components)
    )
    return {
        "family": request.family,
        ...
        **asdict(observation),
        "validation": {
            ...
            "overlapPairPreview": [
                list(pair)
                for pair in validation.overlapping_pairs[:DEFAULT_VALIDATION_OVERLAP_PREVIEW]
            ],
