...
def resolve_sweep_request(args: argparse.Namespace) -> ResolvedRenderReviewSweepRequest:
    ...
    if profile.patch_depth is not None:
        if cell_sizes is not None:
            _parser_error(parser, "--cell-sizes is not valid for patch-depth render-review profiles.")
        ...
        if patch_depths is not None:
            _parser_error(parser, "--patch-depths is not valid for cell-size render-review profiles.")
        ...
        _parser_error(parser, f"Render-review profile {profile.name!r} does not declare a patch depth or cell size.")
...
def sweep_case_dir_name(
    *,
    ...
    sizing_fragment = (
        ...
        else f"size-{cell_size}"
    )
    return f"{index:03d}-host-{host}-theme-{theme}-{sizing_fragment}"


def expand_sweep_cases(request: ResolvedRenderReviewSweepRequest) -> tuple[RenderReviewSweepCase, ...]:
...
def extract_sweep_case_metrics(summary_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "browserTopologyCellCount": (
            browser_state.get("topologyCellCount")
    ...
    try:
        for case in cases:
            ...
            manifest["cases"].append(
                build_sweep_case_record(case=case, run=run)
            )
        exit_status = "success"
