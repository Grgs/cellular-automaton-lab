...
| Newer substitution aperiodic | `hat-monotile` | PASS | PASS | Mixed, local-reference stronger | Hat is connected, overlap-clean, and hole-free under the canonical sample checks, preserves the reflected-neighbor chirality pattern used by the literature verifier, and now also matches a checked-in rooted local-reference fixture. |
| Experimental aperiodic | `chair` | PASS | PASS | Mixed, metadata + local-reference | Kept in the `Experimental` picker group while browser-visible pattern correctness is re-verified; current automated checks now cover the true chair substitution through exact depth totals, orientation-token metadata, and rooted local-reference fixtures. |
...
- Broaden browser-visible rendering-bounds checks and re-verify the experimental families visually before promoting them out of the picker's `Experimental` group.
