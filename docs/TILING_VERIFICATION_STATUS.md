...
| Newer substitution aperiodic | `shield` | PASS | PASS | Mixed, decoration-aware canonical-patch + local-reference + browser-visible render check | Verified with decoration-token metadata, rooted decorated local-reference anchors, an exact decorated depth-1 canonical patch fixture, representative render-bounds coverage, and a settled-render Playwright gate. It remains in `Experimental` because manual visible review still does not justify promotion. |
