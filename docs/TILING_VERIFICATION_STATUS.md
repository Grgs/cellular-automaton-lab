...
| Newer substitution aperiodic | `square-triangle` | PASS | PASS | Mixed, depth-3 canonical | Square-Triangle is now validated on a denser depth-3 canonical sample cut from the cleaned central literature component, so the default patch is connected, overlap-clean, hole-free, and exact on kind counts, adjacency vocabulary, metadata diversity, and signature. |
