...
| Newer substitution aperiodic | `hat-monotile`, `chair`, `shield` | PASS | PASS | Mixed, stronger | Hat uses a deterministic metatile-derived hat projection, Chair exposes a deterministic multiscale chair hierarchy, and Shield now uses a connected cluster-lattice patch. All three pass canonical topology-graph contiguity checks in addition to strict overlap checks. |
| Newer substitution aperiodic | `pinwheel` | PASS | FAIL | Mixed, contiguity gap exposed | The hardened literature verifier now blocks Pinwheel because its canonical sample splits into multiple disconnected components in the topology neighbor graph, even though geometric sanity and overlap checks still pass. |
