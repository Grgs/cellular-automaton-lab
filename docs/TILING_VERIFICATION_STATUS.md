...
| Experimental aperiodic | `chair` | PASS | PASS | Mixed, area-hierarchy + local-reference | Kept in the `Experimental` picker group while browser-visible pattern correctness is re-verified; current automated checks now cover the multiscale chair hierarchy through exact polygon-area frequencies plus rooted local-reference fixtures. |
