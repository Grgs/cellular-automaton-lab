...
| Newer substitution aperiodic | `square-triangle`, `shield` | PASS | FAIL | Mixed, surface-hole gap exposed | These families are connected and overlap-clean, but the canonical samples still form enclosed empty holes in the merged polygon surface. The hardened literature verifier now treats those surrounded gaps as blocking failures. |
...
- Repair `square-triangle` and `shield` so their canonical samples do not enclose empty holes.
