...
| Newer substitution aperiodic | `spectre`, `taylor-socolar`, `sphinx`, `robinson-triangles`, `tuebingen-triangle`, `square-triangle` | PASS | PASS | Mixed | Stronger than render checks, but still mostly based on low-depth counts, metadata, adjacency, signatures, and now contiguity checks rather than full substitution-matrix proofs. Global overlap validation is strict, and the representative render-space overlap fixtures are clean. |
| Newer substitution aperiodic | `hat-monotile`, `chair` | PASS | PASS | Mixed, stronger | Hat uses a deterministic metatile-derived hat projection, Chair exposes a deterministic multiscale chair hierarchy, and both now pass canonical topology-graph contiguity checks in addition to strict overlap checks. |
| Newer substitution aperiodic | `shield`, `pinwheel` | PASS | FAIL | Mixed, contiguity gap exposed | The hardened literature verifier now blocks these families because their canonical samples split into multiple disconnected components in the topology neighbor graph, even though geometric sanity and overlap checks still pass. |
...
- Repair `shield` and `pinwheel` so their canonical samples are contiguous in the topology neighbor graph.
