...
| Newer substitution aperiodic | `spectre`, `taylor-socolar`, `sphinx`, `robinson-triangles`, `tuebingen-triangle`, `square-triangle` | PASS | PASS | Mixed | Stronger than render checks, but still mostly based on low-depth counts, metadata, adjacency, and signatures rather than full substitution-matrix proofs. Global overlap validation is now strict, and the representative render-space overlap fixtures are clean. |
| Newer substitution aperiodic | `hat-monotile`, `chair`, `shield`, `pinwheel` | PASS | PASS | Mixed, stricter staged graduated to pass | Hat now uses a deterministic metatile-derived hat projection, Chair exposes a deterministic multiscale chair hierarchy, Shield carries recursive decoration-state variants, and Pinwheel stays on the exact-affine inflation path. Global overlap validation is now strict, and the representative render-space overlap fixtures are clean. |
