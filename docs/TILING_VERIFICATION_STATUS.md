...
| Newer substitution aperiodic | `hat-monotile`, `chair`, `shield`, `pinwheel` | PASS | PASS | Mixed, stricter staged graduated to pass | Hat now uses a deterministic multi-hat root cluster with the expected reflected-neighbor local pattern, Chair now exposes a deterministic multiscale chair hierarchy, Shield now carries recursive decoration-state variants, and Pinwheel now expands on the exact-affine inflation path instead of subdividing inside a fixed rectangle. |
