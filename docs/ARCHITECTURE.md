...
Geometric validation and literature verification are now separate concerns. `backend/simulation/topology_validation.py` still answers “is this topology internally sane?”, while `backend/simulation/literature_reference_specs.py` and `backend/simulation/literature_reference_verification.py` encode source-backed substitution invariants, waiver/graduation policy, and exact-affine checks for covered aperiodic families such as Penrose, Ammann-Beenker, Spectre, Taylor-Socolar, Chair, Robinson, Hat, Tuebingen Triangle, Square-Triangle, Shield, and Pinwheel. This lets CI report mathematically meaningful deviations separately from geometric sanity.
