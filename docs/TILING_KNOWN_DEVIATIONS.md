...
- `penrose-p1-pentagon-diamond` is built by the de Bruijn pentagrid construction (de Bruijn 1981; Pattern Collider by Aatish Bhatia) followed by a P3 → P1 vertex-merge pass, implemented in `backend/simulation/aperiodic_penrose_multigrid.py`. Stage 1 runs the pentagrid with non-uniform offsets `(0.3, 0.4, 0.5, 0.6, 0.7)` to get a regular Penrose P3 rhomb tiling distributed across the patch without a central singularity. Stage 2 identifies "sun" vertices (5 thick rhombs sharing a 72° apex) and "star" vertices (5 thin rhombs sharing a 36° apex) in the rhomb output and collapses each into a single P1 cell — a decagonal pentagon-region cluster (`p1-pentagon`) for suns, a 10-vertex pentagram (`p1-star`) for stars. Unmerged thick rhombs remain as `p1-pentagon` cells (the rhomb-region MLD representative of the pentagonal P1 prototile) and thin rhombs as `p1-diamond` cells (geometrically exact P1 diamonds). Cells are intrinsically gap-free, edge-matched, and connected at every depth, with depth-to-cell-count sequence 64 / 156 / 416 / 1136 / ... governed by the `1.6 * φ^d` patch radius. The 4th canonical P1 prototile (boats / "justice caps", 7-vertex shapes) is not yet produced — that emerges from a 3-rhomb vertex configuration the current vertex-merge pass does not yet detect. Status is `canonical_patch`: the underlying tiling is the canonical de Bruijn pentagrid dual, equivalent to (and now visually similar to) Penrose's 1974 P1.
