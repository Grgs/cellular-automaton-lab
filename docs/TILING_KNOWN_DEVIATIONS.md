...
- `penrose-p3-rhombs` and `penrose-p3-rhombs-vertex` are still generated by the de Bruijn pentagrid construction in `backend/simulation/penrose.py`. They produce a valid canonical Penrose rhomb tiling but the depth-to-cell-count growth (5/10/24/66 at depths 0..3) is governed by the bounding-box crop rather than the canonical phi^2 substitution eigenvalue. `penrose-p2-kite-dart` and `robinson-triangles` were rewritten in [PENROSE_CANONICAL_SUBSTITUTION_PLAN.md](PENROSE_CANONICAL_SUBSTITUTION_PLAN.md) to use the canonical Robinson half-tile substitution; the analogous P3 rewrite (acute halves paired at short edges into thin rhombs, obtuse halves paired at long edges into thick rhombs) does not produce a clean recursive tiling under the same substitution rules from any rhomb-star or sun seed examined so far, so P3 has been left on the pentagrid generator for now. P3 still tiles the plane correctly, just with a non-substitution depth interpretation.
