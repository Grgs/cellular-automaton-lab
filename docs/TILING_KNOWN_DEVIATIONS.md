...
- `penrose-p1-pentagon-diamond` ships in a hybrid representation: the topology and adjacency graph mirror the canonical Penrose pentagrid in `backend/simulation/penrose.py` exactly (one cell per pentagrid rhomb, same neighbour edges, same depth-to-cell-count sequence 5/10/24/66 as P3), but the rendered geometry is rewritten so the patch visually presents Penrose's P1 prototile shapes. Thin rhombs render as diamonds verbatim (the thin Penrose rhomb IS the P1 diamond). Thick rhombs render as regular pentagons inscribed in the rhomb, sized to fit the short axis with one vertex pointing along the long diagonal; this leaves visible thin lens-shaped gaps along each thick-rhomb perimeter that read as the characteristic P1 pentagonal pattern. The rendered surface therefore has enclosed gaps by design (`require_hole_free_surface=False` in the reference spec), but adjacency / connectivity / cell counts continue to match the canonical pentagrid. Status is `canonical_patch`. A full 4-prototile P1 substitution with literal pentagon / star / boat / diamond cells remains a future-work item.
