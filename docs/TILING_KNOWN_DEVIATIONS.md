...
- `pinwheel-2-1` is the two-prototile pinwheel variant from the Bielefeld Tilings Encyclopedia (https://tilings.math.uni-bielefeld.de/substitution/pinwheel-2-1/). The seed is a single 1:4:√17 right triangle in canonical (small-angle, right-angle, large-angle) vertex order; each parent subdivides into one small child at the right-angle corner (scale 1/√17) plus four large children filling the rest (scale 2/√17), so the depth-d total is 5^d. All construction points (foot of altitude at `(64/17, 16/17)` and the three midpoints) are rational, so the generator runs in exact `Fraction` arithmetic and grandchildren are similar to the parent at every depth. The two prototile sizes are emitted as distinct cell kinds (`pinwheel-2-1-small-triangle`, `pinwheel-2-1-large-triangle`) so renderer, palette, and rules can distinguish them. Status is `exact_affine`; the family stays in `Experimental` until a manual visual review accepts the rendered patch against the published pinwheel-2-1 substitution.
