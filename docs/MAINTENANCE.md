...
The first clean public release line started at `v0.1.0` and continues as an ongoing preview series; `v0.5.0` is the current shipped tag. The wall-first UI overhaul (comparison wall as the landing view, `#/lab` editor in the same one-page shell with a lazily booted controller, live focus-pane fork, Split View retired) targets `v0.6.0`; it is unreleased on the tagged demo, so its route and layout changes ship as "new in 0.6.0" rather than breaking changes — only `#share=` board links predate it and stay working. The public release surface is:
...
- `requirements-lock.in` / `requirements-lock.txt`: the separately hash-pinned `pip-tools` environment used to regenerate the application locks
...
Use the repository command for routine freshness checks and upgrades:
...
To upgrade a single pinned package, edit the matching `.in` file and run
`python -m tools dependencies update`. To change the lock compiler itself, edit
`requirements-lock.in`, regenerate `requirements-lock.txt` with a known-good Python
3.13 environment, and then run the update command to prove it can bootstrap itself.
