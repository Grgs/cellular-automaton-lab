...
- If we revisit `dodecagonal-square-triangle`, continue from the new third-pass inflation probe with explicit line/offset equation inference and supertile-composition search beyond the current snapped literature-vector reconstruction, rooted local-reference anchors, exact public canonical patch fixtures, and exact tile-family/orientation/chirality/degree distributions at depths `1` and `3`.
