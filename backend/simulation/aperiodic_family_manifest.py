...
APERIODIC_FAMILY_MANIFEST: dict[str, AperiodicFamilyManifestEntry] = {
    ...
    PENROSE_P1_GEOMETRY: AperiodicFamilyManifestEntry(
        ...
        # in the all-zero pentagrid manifestation. The ``p1-pentagon-cluster``
        # kind is the 10-vertex decagonal cell emitted at each sun-vertex
        # merge (canonical P1 pentagon position); ``p1-pentagon`` is reserved
        # for unmerged thick rhombs that don't participate in any cluster
        # (rhomb-region MLD representatives of the pentagonal P1 prototile).
        implementation_status="canonical_patch",
        public_cell_kinds=(
            P1_PENTAGON_KIND,
