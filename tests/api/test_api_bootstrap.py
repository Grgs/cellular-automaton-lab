...
class ApiBootstrapTests(ApiTestCase):
    def test_bootstrap_endpoint_returns_shared_runtime_payload(self) -> None:
        ...
        self.assertTrue(any(entry["tiling_family"] == "penrose-p3-rhombs" for entry in payload["aperiodic_families"]))
        self.assertTrue(any(entry["experimental"] for entry in payload["aperiodic_families"]))
        self.assertTrue(all("public_cell_kinds" in entry for entry in payload["aperiodic_families"]))
