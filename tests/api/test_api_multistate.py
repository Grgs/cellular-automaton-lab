...
class ApiMultistateTests(ApiTestCase):
    def test_random_reset_is_rejected_for_nonrandomizable_rules(self) -> None:
        bad_wireworld = self.client.post('/api/control/reset', json={'rule': 'wireworld', 'randomize': True})
        bad_whirlpool = self.client.post('/api/control/reset', json={'rule': 'whirlpool', 'randomize': True})
        bad_hexwhirlpool = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'hex', 'adjacency_mode': 'edge', 'width': 10, 'height': 6, 'patch_depth': 0},
            'rule': 'hexwhirlpool',
            'randomize': True,
        })
    ...
    def test_whirlpool_eye_cells_reignite_instead_of_dying_when_supported(self) -> None:
        self.reset_simulation(width=9, height=9, rule='whirlpool', randomize=False)
        painted = self.client.post('/api/cells/set-many', json={'cells': [
            {'id': 'c:4:4', 'state': 3},
            {'id': 'c:4:3', 'state': 1},
        ]})
        self.assertEqual(painted.status_code, 200)
    ...
    def test_hexwhirlpool_eye_cells_reignite_instead_of_dying_when_supported(self) -> None:
        reset = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'hex', 'adjacency_mode': 'edge', 'width': 9, 'height': 7, 'patch_depth': 0},
            'speed': 5,
            'rule': 'hexwhirlpool',
            'randomize': False,
        })
    ...
    def test_switching_between_wireworld_and_highlife_coerces_grid_and_updates_rule_metadata(self) -> None:
        self.client.post('/api/config', json={'rule': 'wireworld'})
        painted = self.client.post('/api/cells/set-many', json={'cells': [
            {'id': 'c:1:1', 'state': 3},
            {'id': 'c:2:1', 'state': 2},
        ...
        self.assertEqual([cell_state['value'] for cell_state in highlife_payload['rule']['states']], [0, 1])
    ...
    def test_invalid_wireworld_batch_does_not_apply_partial_updates(self) -> None:
        ...
        invalid_batch = self.client.post('/api/cells/set-many', json={'cells': [
            {'id': 'c:1:1', 'state': 3},
            {'id': 'c:2:1', 'state': 9},
        ]})
    ...
    def test_trilife_can_be_selected_on_triangle_and_square(self) -> None:
        triangle_reset = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'triangle', 'adjacency_mode': 'edge', 'width': 9, 'height': 7, 'patch_depth': 0},
            'speed': 5,
            'randomize': False,
        })
        ...
        square_reset = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'square', 'adjacency_mode': 'edge', 'width': 9, 'height': 7, 'patch_depth': 0},
            'speed': 5,
            'rule': 'conway',
            'randomize': False,
        })
    ...
    def test_hexwhirlpool_alias_selects_whirlpool_on_hex_and_square(self) -> None:
        hex_reset = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'hex', 'adjacency_mode': 'edge', 'width': 9, 'height': 7, 'patch_depth': 0},
            'speed': 5,
            'randomize': False,
        })
    ...
    def test_reset_preserves_rectangular_whirlpool_dimensions(self) -> None:
        square_rule = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'square', 'adjacency_mode': 'edge', 'width': 12, 'height': 8, 'patch_depth': 0},
            'speed': 5,
            'rule': 'whirlpool',
            'randomize': False,
        })
        hex_rule = self.client.post('/api/control/reset', json={
            'topology_spec': {'tiling_family': 'hex', 'adjacency_mode': 'edge', 'width': 11, 'height': 7, 'patch_depth': 0},
            'speed': 5,
            'rule': 'hexwhirlpool',
            'randomize': False,
        })
    ...
    def test_switching_into_whirlpool_rule_preserves_rectangular_dimensions_and_overlap(self) -> None:
    ...
    def test_archlife_can_be_selected_on_archimedean_and_square(self) -> None:
        arch_reset = self.client.post('/api/control/reset', json={
