...
class ApiPersistenceTests(ApiTestCase):
    def test_app_ignores_preexisting_persisted_snapshot_with_retired_version(self) -> None:
        ...
        self.assertEqual(
            payload["topology_spec"]["width"], APP_DEFAULTS["simulation"]["topology_spec"]["width"]
        )
        self.assertEqual(
            payload["topology_spec"]["height"],
            APP_DEFAULTS["simulation"]["topology_spec"]["height"],
        )
    ...
    def test_app_boots_from_preexisting_persisted_snapshot_v5(self) -> None:
        ...
        index_by_id = {cell["id"]: index for index, cell in enumerate(payload["topology"]["cells"])}
    ...
    def test_clean_shutdown_persists_latest_running_state_and_restores_paused(self) -> None:
        ...
        before_shutdown = self.wait_for_state(
            lambda state: state["running"] and state["generation"] >= 1
        )
        ...
        self.assertEqual(
            after_restart["topology_spec"]["width"], before_shutdown["topology_spec"]["width"]
        )
        self.assertEqual(
            after_restart["topology_spec"]["height"], before_shutdown["topology_spec"]["height"]
        )
    ...
    def test_archimedean_3464_state_restores_after_app_recreation(self) -> None:
        reset = self.client.post(
            ...
            json={
                "topology_spec": {
                    "tiling_family": "archimedean-3-4-6-4",
                    "adjacency_mode": "edge",
                    "width": 2,
                    "height": 2,
                    "patch_depth": 0,
