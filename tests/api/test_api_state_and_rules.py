...
class ApiStateAndRulesTests(ApiTestCase):
    ...
    def test_rules_and_initial_state(self) -> None:
        ...
        self.assertEqual(
            [cell_state["value"] for cell_state in whirlpool["states"]], [0, 1, 2, 3, 4]
        )
    ...
    def test_topology_endpoint_and_penrose_state_contract(self) -> None:
        ...
        self.assertEqual(state["topology_spec"]["adjacency_mode"], "edge")
        self.assertEqual(
            state["topology_spec"]["patch_depth"],
            maximum_patch_depth_for_tiling_family(PENROSE_GEOMETRY),
        )
        self.assertFalse(state["running"])
        self.assertEqual(state["generation"], 0)
        self.assertEqual(state["rule"]["name"], "life-b2-s23")
    ...
    def test_ammann_beenker_patch_depth_uses_family_cap(self) -> None:
        ...
        self.assertEqual(
            state["topology_spec"]["patch_depth"],
            maximum_patch_depth_for_tiling_family("ammann-beenker"),
        )
        self.assertEqual(topology["topology_spec"], state["topology_spec"])
        self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
        self.assertGreater(len(topology["cells"]), 100)
    ...
    def test_spectre_patch_depth_uses_generic_rule_and_family_cap(self) -> None:
        ...
        self.assertEqual(
            state["topology_spec"]["patch_depth"], maximum_patch_depth_for_tiling_family("spectre")
        )
        self.assertEqual(state["rule"]["name"], "life-b2-s23")
        self.assertEqual(topology["topology_spec"], state["topology_spec"])
        self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
    ...
    def test_taylor_socolar_patch_depth_uses_generic_rule_and_family_cap(self) -> None:
        ...
        self.assertEqual(
            state["topology_spec"]["patch_depth"],
            maximum_patch_depth_for_tiling_family("taylor-socolar"),
        )
        self.assertEqual(state["rule"]["name"], "life-b2-s23")
        self.assertEqual(topology["topology_spec"], state["topology_spec"])
        self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
    ...
    def test_sphinx_patch_depth_uses_generic_rule_and_family_cap(self) -> None:
        ...
        self.assertEqual(
            state["topology_spec"]["patch_depth"], maximum_patch_depth_for_tiling_family("sphinx")
        )
        self.assertEqual(state["rule"]["name"], "life-b2-s23")
        self.assertEqual(topology["topology_spec"], state["topology_spec"])
        self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
    ...
    def test_chair_patch_depth_uses_generic_rule_and_family_cap(self) -> None:
        ...
        self.assertEqual(
            state["topology_spec"]["patch_depth"], maximum_patch_depth_for_tiling_family("chair")
        )
        self.assertEqual(state["rule"]["name"], "life-b2-s23")
        self.assertEqual(topology["topology_spec"], state["topology_spec"])
        self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
    ...
    def test_robinson_triangles_patch_depth_uses_generic_rule_and_family_cap(self) -> None:
        ...
        self.assertEqual(
            state["topology_spec"]["patch_depth"],
            maximum_patch_depth_for_tiling_family("robinson-triangles"),
        )
        self.assertEqual(state["rule"]["name"], "life-b2-s23")
        self.assertEqual(topology["topology_spec"], state["topology_spec"])
        self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
        self.assertGreater(len(topology["cells"]), 100)
        self.assertEqual(
            {cell["kind"] for cell in topology["cells"]}, {"robinson-thick", "robinson-thin"}
        )
    ...
    def test_new_aperiodic_wave_tilings_reset_with_metadata_and_expected_caps(self) -> None:
        ...
        for geometry, expected_kinds in cases:
            with self.subTest(geometry=geometry):
                ...
                self.assertEqual(
                    state["topology_spec"]["patch_depth"],
                    maximum_patch_depth_for_tiling_family(geometry),
                )
                self.assertEqual(state["rule"]["name"], "life-b2-s23")
                self.assertEqual(topology["topology_spec"], state["topology_spec"])
                self.assertEqual(len(topology["cells"]), len(state["cell_states"]))
                ...
                self.assertTrue(
                    all(cell.get("tile_family") is not None for cell in topology["cells"])
                )
                if geometry in {"hat-monotile", "tuebingen-triangle", "pinwheel"}:
                    self.assertTrue(
                        all(cell.get("orientation_token") is not None for cell in topology["cells"])
                    )
                    self.assertTrue(
                        all(cell.get("chirality_token") is not None for cell in topology["cells"])
                    )
                if geometry == "dodecagonal-square-triangle":
                    self.assertTrue(
                        any(
                            cell["kind"] == "dodecagonal-square-triangle-square"
                ...
                if geometry == "shield":
                    self.assertTrue(
                        any(cell["kind"] == "shield-square" for cell in topology["cells"])
                    )
                    self.assertTrue(
                        all(cell.get("orientation_token") is not None for cell in topology["cells"])
                    )
    ...
    def test_legacy_b2_s23_rule_names_are_rejected(self) -> None:
        for legacy_name, tiling_family in (
            ...
            with self.subTest(rule=legacy_name):
                ...
                self.assertEqual(
                    reset.get_json(), {"error": "'rule' must reference a known rule module."}
                )
