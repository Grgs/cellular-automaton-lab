...
class ApiControlTests(ApiTestCase):
    ...
    def test_dimension_changing_config_update_pauses_running_simulation(self) -> None:
        ...
        response = self.client.post(
            "/api/config", json={"topology_spec": {"width": 12, "height": 9}}
        )
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
    ...
    def test_reset_and_config_return_exact_state_payloads(self) -> None:
        resized = self.client.post(
            "/api/config", json={"topology_spec": {"width": 12, "height": 8}}
        )
        self.assertEqual(resized.status_code, 200)
        resized_payload = resized.get_json()
        self.assertIn("topology", resized_payload)
        self.assertEqual(
            resized_payload["topology_revision"], resized_payload["topology"]["topology_revision"]
        )
        ...
        self.assertEqual(
            reset_payload["topology_revision"], reset_payload["topology"]["topology_revision"]
        )
    ...
    def test_reset_can_switch_to_kagome_geometry(self) -> None:
        ...
        self.assertNotIn("grid", payload)
        self.assertEqual(
            payload["topology"]["topology_spec"]["tiling_family"], "trihexagonal-3-6-3-6"
        )
