...
try:
    ...
    from backend.simulation.topology_family_manifest import (
        ...
        UNIFORM_2_2_3122_34312_GEOMETRY,
        UNIFORM_2_10_GEOMETRY,
        UNIFORM_2_12_GEOMETRY,
        UNIFORM_2_13_GEOMETRY,
        UNIFORM_2_18_GEOMETRY,
        UNIFORM_2_19_V1_36_346_GEOMETRY,
        UNIFORM_3_4_36_3262_63_GEOMETRY,
...
class SimulationTopologyPeriodicTests(unittest.TestCase):
    ...
    def test_uniform_2_18_has_expected_face_mix_and_valid_interior(self) -> None:
        topology = build_topology(UNIFORM_2_18_GEOMETRY, 3, 3)

        self.assertEqual(topology.cell_count, 198)
        self.assertEqual(
            Counter(cell.kind for cell in topology.cells),
    ...
    def test_uniform_2_12_has_expected_face_mix_and_valid_interior(self) -> None:
        topology = build_topology(UNIFORM_2_12_GEOMETRY, 3, 3)

        self.assertEqual(topology.cell_count, 201)
        self.assertEqual(
            Counter(cell.kind for cell in topology.cells),
            Counter({"triangle": 156, "hexagon": 45}),
        )
        validation = validate_topology(topology)
        self.assertTrue(validation.is_valid, "\n".join(validation.summary_lines()))
        self.assertFalse(validation.edge_multiplicity_issues)
    ...
    def test_uniform_2_19_v1_has_expected_face_mix_and_valid_interior(self) -> None:
        topology = build_topology(UNIFORM_2_19_V1_36_346_GEOMETRY, 3, 3)

        self.assertEqual(topology.cell_count, 490)
        self.assertEqual(
            Counter(cell.kind for cell in topology.cells),
            Counter({"triangle": 444, "hexagon": 46}),
        )
        validation = validate_topology(topology)
        self.assertTrue(validation.is_valid, "\n".join(validation.summary_lines()))
        self.assertFalse(validation.edge_multiplicity_issues)
    ...
    def test_uniform_3_4_has_expected_face_mix_and_valid_interior(self) -> None:
        topology = build_topology(UNIFORM_3_4_36_3262_63_GEOMETRY, 3, 3)

        self.assertEqual(topology.cell_count, 348)
        self.assertEqual(
            Counter(cell.kind for cell in topology.cells),
            Counter({"triangle": 228, "hexagon": 120}),
        )
        validation = validate_topology(topology)
        self.assertTrue(validation.is_valid, "\n".join(validation.summary_lines()))
        self.assertFalse(validation.edge_multiplicity_issues)
    ...
    def test_uniform_2_10_has_expected_face_mix_and_valid_interior(self) -> None:
        topology = build_topology(UNIFORM_2_10_GEOMETRY, 3, 3)

        self.assertEqual(topology.cell_count, 175)
        self.assertEqual(
            Counter(cell.kind for cell in topology.cells),
            Counter({"triangle": 129, "hexagon": 46}),
        )
        validation = validate_topology(topology)
        self.assertTrue(validation.is_valid, "\n".join(validation.summary_lines()))
        self.assertFalse(validation.edge_multiplicity_issues)
    ...
    def test_uniform_2_2_has_expected_face_mix_and_valid_interior(self) -> None:
        topology = build_topology(UNIFORM_2_2_3122_34312_GEOMETRY, 3, 3)

        self.assertEqual(topology.cell_count, 61)
        self.assertEqual(
            Counter(cell.kind for cell in topology.cells),
            Counter({"triangle": 36, "dodecagon": 16, "square": 9}),
        )
        validation = validate_topology(topology)
        self.assertTrue(validation.is_valid, "\n".join(validation.summary_lines()))
        self.assertFalse(validation.edge_multiplicity_issues)
